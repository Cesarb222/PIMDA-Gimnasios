...
@Composable
fun ViewRegistrosView(navController: NavController,userVM: UserViewModel,ejercicioVM: EjercicioViewModels,registroVM: RegistroViemModel){
    Scaffold(topBar = {
        //barra superior
        ...
        if (USUARIO!=null) {
            ViewRegistrosContent(navController, it, userVM,ejercicioVM,registroVM)
        }else{
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ViewRegistrosContent(navController: NavController,paddingValues: PaddingValues,
                       userVM: UserViewModel,ejercicioVM: EjercicioViewModels,
                       registroVM: RegistroViemModel
) {
    registroVM.getMisRegistros(USUARIO!!.id)
    val misRegistros = registroVM.misRegistros.collectAsState().value
